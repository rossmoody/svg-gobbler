  ...
  svgInfo = svgInfo.map(i => {
    ...
    if (i.element.hasAttribute('src')) {
      ...
      ajax.onload = function (e) {
        let string = parser.parseFromString(ajax.responseText, 'image/svg+xml')
          .children[0]
...
createButtons = svgInfo => {
  ...
  container.setAttribute('class', 'svgeezy__container')
  document.body.appendChild(container)

  svgInfo.forEach((svg, id) => {
    const btnContainer = document.createElement('div')
    container.appendChild(btnContainer)
    btnContainer.setAttribute('class', 'svgeezy__btn__container')
    ...
    btn.setAttribute('class', 'svgeezy__btn')
